...
public class SlaveBossManager {
    private Map<UUID, Boss> bosses = new HashMap<>();

    protected Boss lookup(UUID entity) {
        return bosses.get(entity);
    }

    protected void silentBind(Boss boss) {
        bosses.put(boss.getEntityID(), boss);
    }

    protected boolean silentUnbind(Boss boss) {
        return bosses.remove(boss.getEntityID()) != null;
    }
}
