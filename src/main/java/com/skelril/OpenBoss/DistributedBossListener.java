...
public class DistributedBossListener implements Listener {

    private DistributedBossManager manager;

    public DistributedBossListener(DistributedBossManager manager) {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        Boss boss = lookup(event.getEntity());
        if (boss != null) {
            for (SlaveBossManager slave : manager.getManagers()) {
                if (manager.unbind(boss, slave)) {
                    break;
                }
            }
        }
    ...
    public Boss lookup(Entity entity) {
        ...
        if (entity instanceof LivingEntity) {
            for (SlaveBossManager slave : manager.getManagers()) {
                boss = manager.updateLookup((LivingEntity) entity, slave);
                if (boss != null) {
                    break;
                }
            }
